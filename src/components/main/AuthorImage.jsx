...
class AuthorImage extends Component {
  ...
  componentDidMount() {
    this.setState({ loaded: false }, () => this.setImage(this.props));
  }

  componentWillReceiveProps(nextProps) {
    const { author, allImages } = this.props;
    const loaded =
      author === nextProps.author && Boolean(allImages.get(author));
  ...
  setImage(props) {
    ...
    const imgWidth =
      window.innerWidth < responsiveBreakpoint ? window.innerWidth : 250;
